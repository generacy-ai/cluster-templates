...
**Answer**: B) PRs targeting main/develop only. The workflow will only trigger on PRs targeting `main` or `develop`, matching the push trigger scope.
...
**Answer**: A) No path filtering. CI will run on every qualifying push/PR regardless of which files changed, for simpler configuration and guaranteed coverage.
...
**Answer**: A) Fail-fast enabled (default). The matrix will use the default fail-fast behavior, cancelling remaining jobs when one fails for faster feedback.
...
**Answer**: A) No concurrency control. All triggered runs will complete independently, keeping the configuration simpler.
...
**Answer**: A) No dummy files. The workflow will trust that `docker compose config` only validates structure, keeping the configuration simpler.
